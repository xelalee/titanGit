...
//var MailFormat = /^\w+([\._-]\w+)*\@\w+([_-]\w+)?.\w+([_-]\w+)?(.\w+([_-]\w+)?)?$/;
  var MailFormat = /^\w+([\._-]\w+)*\@\w+([_-]\w+)?.\w+([_-]\w+)?(.\w+([_-]\w+)?)?(.\w+([_-]\w+)?)?(.\w+([_-]\w+)?)?(.\w+([_-]\w+)?)?(.\w+([_-]\w+)?)?(.\w+([_-]\w+)?)?$/;
...
function filterEmailInput(event){
	event.target.value = event.target.value.replace(/[^0-9a-zA-Z_@\\-\\.\n]/g,"");
}
